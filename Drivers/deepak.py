...
def deepak_sample(dependency,run_time_params,step_data,file_attachment,temp_q,screen_capture):
    sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
    try:
        CommonUtil.ExecLog(sModuleInfo, "Enter: Step - Search an Item on Amazon", 1)
        step_data = [ [ ( 'content-desc' , 'element parameter' , 'Search' , False , False ) , ( 'click' , 'appium action' , 'click' , False , False ) ]  , [ ( 'content-desc' , 'element parameter' , 'Search query' , False , False ) , ( 'text' , 'appium action' , 'abcd' , False , False ) ]  , [ ( 'sleep' , 'appium action' , '10' , False , False ) ]  , [ ( 'close' , 'appium action' , 'close' , False , False ) ] ]
